...
export default class TossPaymentPopup {
  constructor(classId, spaceName, email, customerKey) {
    this.successUrl = `${process.env.DB}/payment/successpage?classId=${encodeURIComponent(classId)}&spaceName=${encodeURIComponent(spaceName)}&email=${encodeURIComponent(email)}`;
